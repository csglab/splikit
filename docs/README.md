...
We are useing the raw tab file for the junction abundace and will apply ad-hoc filtartion troughout the pipeline. For more information about how to build genome index and mapping using STAR please take a look at examples codes for [building index](../example_star_solo_outout/build_index.sh) and [mapping the fastq using STARsolo](../example_star_solo_outout/star_mapping.sh).
...
-   `STARsolo_SJ_dirs` A character vector or list of strings representing the paths to STARsolo SJ directories. Each directory should contain the raw splicing junction output files. sample_ids
...
**Arguments:** - `junction_ab_object` A named list that function `multigedi_make_junction_ab` returns where each element represents a sample's junction abundance data. Each element must contain eventdata\` and a sparse matrix
...
There is no need to assign a variable for this command; it will automatically store the output in the `.GlobalEnv` as `object_names_test` and `object_names_train`.<br/> After performing the count splitting, derive the M2 matrix from the M1 matrix. This step should be executed only once for each individual M1 matrix, regardless of whether it is original, test, or training data.
...
-   `merge_counts` A logical flag that defaults to `FALSE`. When set to `TRUE`, the function merges all spliced and unspliced count matrices across samples into two larger matrices—one for spliced counts and one for unspliced counts. If `merge_counts` is `FALSE`, the function returns a list where each sample is represented by its own pair of `dgCMatrix` objects: one for spliced counts and one for unspliced counts.

In some cases, you may need to include gene names and IDs associated with each junction for downstream analyses. This additional information can be easily added to the existing event data using the function `multigedi_make_eventdata_plus` and the same GTF file that was used to create the STAR index.

### 5. `multigedi_make_eventdata_plus`

**Example:**

``` r
eventdata_plus <- multigedi_make_eventdata_plus(
    eventdata = m1_obj$event_data, 
    GTF_file_direction = "~/scratch/spatial-data/data/index/gencode.vM36.annotation.gtf")
```

The output should include `gene_id` and `gene_name` columns.

**Arguments:**

-   `eventdata` A data table that can obtained from `multigedi_make_m1` function. For this fucntion it should have these columns:
